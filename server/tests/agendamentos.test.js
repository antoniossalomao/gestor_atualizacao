...
test("AgendamentoRepository - busca por data e filtros rápidos", async (t) => {
  const env = ambiente();
  try {
    const hoje = new Date();
    ...
    const hojeBR = `${dd}/${mm}/${aaaa}`;

    criar(env.service, env.db, { tarefa: "Tarefa Hoje", cliente: "Cliente 1", data: hojeBR, responsavel: "Antonio" });
    criar(env.service, env.db, { tarefa: "Tarefa Ontem", cliente: "Cliente 2", data: "01/01/2020", responsavel: "Marcos", status: "A Fazer" });
    criar(env.service, env.db, { tarefa: "Tarefa Concluída Antiga", cliente: "Cliente 3", data: "01/01/2020", responsavel: "Antonio", status: CONCLUIDO });

    await t.test("busca textual encontra tarefa pela data", () => {
      const res = env.db.agendamentos.list(hojeBR);
      assert.equal(res.total, 1);
      assert.equal(res.rows[0].cliente, "Cliente 1");
    });

    await t.test("filtro rápido 'hoje' traz somente tarefas de hoje", () => {
      const res = env.db.agendamentos.list("", "Todos", { filtroRapido: "hoje" });
      assert.equal(res.total, 1);
      assert.equal(res.rows[0].tarefa, "Tarefa Hoje");
    });

    await t.test("filtro rápido 'atrasadas' traz pendentes anteriores a hoje", () => {
      const res = env.db.agendamentos.list("", "Todos", { filtroRapido: "atrasadas" });
      assert.equal(res.total, 1);
      assert.equal(res.rows[0].tarefa, "Tarefa Ontem");
    });

    await t.test("filtro rápido 'minhas' filtra pelo responsável informado", () => {
      const res = env.db.agendamentos.list("", "Todos", { filtroRapido: "minhas", usuarioNome: "Antonio" });
